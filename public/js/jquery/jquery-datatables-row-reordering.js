            ...
            $("tbody", oTable).disableSelection().sortable({
                ...
                update: function (event, ui) {
                    ...
                    if (properties.sURL != null) {
                        ...
                        //add callback for none url process
                        if ('function' == typeof properties.submitFunction){
                        	var success = properties.submitFunction($dataTable);
                        	if (false ==success){
                        		fnCancelSorting($dataTable, tbody, properties, 1, properties.errorMessage);
                        	}else{
                        		fnMoveRows($dataTable, sSelector, oState.iCurrentPosition, oState.iNewPosition, oState.sDirection, ui.item.context.id, sGroup);
                        	}
                        }else{
