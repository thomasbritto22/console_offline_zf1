...
;(function($){$.fn.camera = function(opts, callback) {
	...
	var defaults = {
		alignment			: 'centerLeft', //topLeft, topCenter, topRight, centerLeft, center, centerRight, bottomLeft, bottomCenter, bottomRight
		...
		easing				: 'linear',	//for the complete list http://jqueryui.com/demos/effect/easing.html
		...
		imagePath			: 'custom_files/',	//he path to the image folder (it serves for the blank.gif, when you want to display videos)
...
////////callbacks
		...
		onEndTransition		: function() {  
								// Define all of our variables so that we can use them later
								var slideImage = $('.cameraSlide.cameracurrent').find('img');
								...
								var parentWidth = slideImage.parent().width();
								var parentHeight = slideImage.parent().height();
								var ratio;
								var newHeight;
								var newWidth;

								// to get the correct aspect ration these are the equations: 
								// newHeight = naturalImageHeight/naturalImageWidth*parentWidth;
								// newWidth = naturalImageWidth/naturalImageHeight*parentHeight;

								// Image is wider and taller that parent div
								if(naturalImageHeight > parentHeight && naturalImageWidth > parentWidth) {
									...
									// Image is shorter than it's width	
										...
										var t = Math.abs(s)/2;
										var u = parentWidth - newWidth;
										var v = Math.abs(u)/2;

										// New height and new width are both bigger than the parent
										if(newHeight > parentHeight && newWidth > parentWidth) {
											slideImage.css({'width': newWidth + 'px', 'height': newHeight + 'px'});
										
										// New height is bigger than parent and new width is smaller than parent
										} else if(newHeight > parentHeight && newWidth < parentWidth) {
											slideImage.css({'width': newWidth + 'px', 'height': parentHeight + 'px', 'margin-left': v + 'px'});	
										
										// New height is shorter than parent and new width is larger than parent
										} else if(newHeight < parentHeight && newWidth > parentWidth) {
								...
								// Image is taller than parent and taller than width
								...
								// Image is wider that parent and wider than height
								...
								// Image is naturally smaller than bother parent width and parent height
								} else {
									var l = parentWidth - naturalImageWidth;
									var m = parentHeight - naturalImageHeight;
									var o = l/2;
	...
	var wrap = $(this).addClass('camera_wrap secondaryBgColor');
	
	wrap.wrapInner(
        '<div class="camera_src" />'
		).wrapInner(
	    '<div class="camera_fakehover contentBgColor" />'
		);
	...
	if(elem.length!=0){
		...
		if($(pagination).length) {
			...
			for (li = 0; li < amountSlide; li++){
				$('.camera_pag_ul',wrap).append('<li class="pag_nav_'+li+'" style="position:relative; z-index:1000"><span><span>'+li+'</span></span></li>');
			}
