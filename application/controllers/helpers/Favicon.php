...
require_once ("../application/soapModels/CatalystService/FileManager.php");

class Helpers_Favicon extends Zend_Controller_Action_Helper_Abstract {
	/**
	 ...
	 */
	function __construct( $file = false, $sizes = array(64) ) {
	...
	public function addImageById($fileId, $size = array(64,64)){
		$sm = new FileManagerSoapModel();
		$imgs = $sm->getCustomFiles('image');
		...
		if(is_array($imgs) &&  count($imgs) !== 0){
			...
			for($i = 0; $i < $imgCount; ++$i){
				if($imgs[$i]->id === $fileId){
					$this->add_image('.'.$imgs[$i]->path, $size);
					return true;	
				}
			}	
		}
	 ...
	 */
	private function add_image( $file, $sizes = array() ) {
