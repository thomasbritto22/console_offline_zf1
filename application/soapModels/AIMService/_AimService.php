...
class AimServiceSoapModel {
    ...
    protected function init () {
        ...
        $serviceName = $this->lcfirst(str_replace("AIMSoapModel", "", get_class($this)));

        // now try to get the SOAP XML for this service
        try{
            $this->client = new Zend_Soap_Client(
                SOAP_AIM_SERVICE_URL . $serviceName .".srv",
                array(
                'soap_version' => SOAP_1_1,
                'wsdl' => SOAP_AIM_SERVICE_URL . $serviceName . ".srv?wsdl",
                )
            );
        }
        catch (Exception $e) {
